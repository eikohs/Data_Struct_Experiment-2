...
int main(void)
{
    ...
    while (1)
    {
        ...
        QueuePtr Que = Create_Queue();//初始化栈和队列
        if (Get_Chars(Sta, Que) == 0)//读取字符串并且存入栈和队列，若无数据输入则特殊处理
        {
        ...
        if (DEBUG&&WHETHER_PRINT)//此段调试用
        {
            PRINT_QUE(Que);
            PRINT_STACK(Sta);
        }
        printf("%s\n", Judge_Ret(Sta, Que) ? "Yes, it is" : "No, it isn't");//判定是否为回文数，输出结果
        Dispose_Que(Que);
        Dispose_Stack(Sta);//清除栈和队列
        if (!DEBUG)//这一段非调试时加入
        {
            printf("Want to run again?(q/Q to quit, y to run again):");
            while ((Flag = getchar()) == '\n') continue;
